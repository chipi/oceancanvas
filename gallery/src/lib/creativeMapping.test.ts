...
describe("creativeToTechnical", () => {
  ...
  it("colour_character boundaries are correct", () => {
    ...
    expect(creativeToTechnical({ ...s, colour_character: 0.0 }).colormap).toBe(
      "arctic",
    );
    expect(creativeToTechnical({ ...s, colour_character: 0.32 }).colormap).toBe(
      "arctic",
    );
    expect(creativeToTechnical({ ...s, colour_character: 0.34 }).colormap).toBe(
      "thermal",
    );
    expect(creativeToTechnical({ ...s, colour_character: 0.67 }).colormap).toBe(
      "otherworldly",
    );
  });
...
describe("technicalToCreative", () => {
  it("round-trips all presets approximately", () => {
    for (const [, preset] of Object.entries(MOOD_PRESETS)) {
      ...
      expect(
        Math.abs(reversed.temporal_weight - preset.temporal_weight),
      ).toBeLessThan(0.15);
    }
  });

  it("maps thermal colormap to mid colour_character", () => {
    const result = technicalToCreative({
      colormap: "thermal",
      opacity: 0.7,
      speed_scale: 1.0,
      particle_count: 2000,
...
describe("creativeToAudio", () => {
  ...
  it("colour_character drives drone waveform across three buckets", () => {
    expect(
      creativeToAudio({ ...MOOD_PRESETS["Becalmed"], colour_character: 0.1 })
        .drone_waveform,
    ).toBe("sine");
    expect(
      creativeToAudio({ ...MOOD_PRESETS["Becalmed"], colour_character: 0.5 })
        .drone_waveform,
    ).toBe("triangle");
    expect(
      creativeToAudio({ ...MOOD_PRESETS["Becalmed"], colour_character: 0.9 })
        .drone_waveform,
    ).toBe("sawtooth");
  });
