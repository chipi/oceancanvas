...
export function ArcEditor({
  ...
  const pathD = useMemo(
    () => buildPath(spec, dominantMomentFrame),
    [spec, dominantMomentFrame],
  );
  ...
  const effectivePeak = useMemo(() => {
    if (
      ...
      return Math.max(
        0.05,
        Math.min(0.95, dominantMomentFrame / (totalFrames - 1)),
      );
    }
  ...
  const onKeyDown = useCallback(
    (e: React.KeyboardEvent<SVGSVGElement>) => {
      ...
      if (e.key === "ArrowLeft")
        next = {
          ...spec,
          peak_position: clamp(spec.peak_position - step, 0.05, 0.95),
          pin_key_moment: false,
        };
        ...
        next = {
          ...spec,
          peak_height: clamp(spec.peak_height + step, 0.1, 1.0),
        };
      ...
      onChange({
        ...next,
        peak_position: round(next.peak_position, 2),
        peak_height: round(next.peak_height, 2),
      });
    },
    [spec, onChange, isDisabled],
  );
      ...
      {!isDisabled && dominantMomentFrame !== null && (
        <label className={styles.pinRow}>
          <input
            ...
            onChange={(e) =>
              onChange({ ...spec, pin_key_moment: e.target.checked })
            }
          />
...
function buildPath(
  ...
  const points = arc.map(
    (v, i) =>
      `${(i * dx).toFixed(2)},${((1 - clamp(v, 0, 1)) * VIEW_H).toFixed(2)}`,
  );
