          ...
          <div
            ...
            onMouseDown={(e) => {
              ...
              const handleMove = (ev: MouseEvent) => {
                ...
                update({
                  energy_x: Math.round(x * 100) / 100,
                  energy_y: Math.round(y * 100) / 100,
                });
              };
              const handleUp = () => {
            ...
            onKeyDown={(e) => {
              const step = 0.05;
              if (e.key === "ArrowRight")
                update({ energy_x: Math.min(1, state.energy_x + step) });
              if (e.key === "ArrowLeft")
                update({ energy_x: Math.max(0, state.energy_x - step) });
              if (e.key === "ArrowUp")
                update({ energy_y: Math.min(1, state.energy_y + step) });
              if (e.key === "ArrowDown")
                update({ energy_y: Math.max(0, state.energy_y - step) });
            }}
          >
            <div className={styles.quadrantGrid} />
            <div className={styles.quadrantLabels}>
              <span className={styles.qLabel} style={{ top: 4, left: 4 }}>
                Ghost
              </span>
              <span className={styles.qLabel} style={{ top: 4, right: 4 }}>
                Storm
              </span>
              <span className={styles.qLabel} style={{ bottom: 4, left: 4 }}>
                Dormant
              </span>
              <span className={styles.qLabel} style={{ bottom: 4, right: 4 }}>
                Becalmed
              </span>
            </div>
            <div
            ...
            <div className={styles.spectrumWrap}>
              <div className={styles.spectrum}>
                <input
                  ...
                  onChange={(e) =>
                    update({ colour_character: Number(e.target.value) / 100 })
                  }
          ...
          <div className={styles.section}>
            ...
            <div className={styles.sliderWrap}>
              <input
                ...
                onChange={(e) =>
                  update({ temporal_weight: Number(e.target.value) / 100 })
                }
