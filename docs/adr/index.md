...
2. The RFC's status flips to *Decided* in `../rfc/index.md` and in `OC_TA.md` (Map).
3. The new ADR is added to the ADRs table in `OC_TA.md` (Map). If it locks a piece of the stack, the **Stack** table in `OC_TA.md` also updates with the ADR pointer.
