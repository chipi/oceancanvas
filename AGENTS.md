...
This file is the canonical orientation document for any AI agent working on the codebase — portable, tool-agnostic. Claude Code reads `CLAUDE.md`, which defers here; other agents (Cursor, ChatGPT, GitHub Copilot Workspace, …) should read this directly. The code conventions here are firm; the doc-system conventions match the structure under `docs/`.
...
- **Tool-specific overlays** — `CLAUDE.md` (Claude Code-specific overrides on top of this file)
